...
class StartupLog(base.BigIntBase):
    ...
    startup_time: Mapped[datetime] = mapped_column(default=func.now(), comment="When the application started")
    shutdown_time: Mapped[datetime | None] = mapped_column(default=None, comment="When the application shut down")

    # Common audit fields
    created_by: Mapped[str] = mapped_column(String(length=64), default=get_login_user, comment="Created by")
    created_at: Mapped[datetime] = mapped_column(default=func.now(), comment="Creation timestamp")
    updated_by: Mapped[str | None] = mapped_column(String(length=64), default=get_login_user, comment="Last updated by")
    updated_at: Mapped[datetime | None] = mapped_column(
        default=func.now(), onupdate=func.now(), comment="Last update timestamp"
    )
